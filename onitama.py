...
class Piece(Enum):
    ...
    def belongs_to(self, player):
        if player == Player.RED:
            return self in [Piece.R_KING, Piece.R_PAWN]
        elif player == Player.BLUE:
            return self in [Piece.B_KING, Piece.B_PAWN]
        else:
            return False

    def is_pawn(self):
        return self in [Piece.R_PAWN, Piece.B_PAWN]

    def is_king(self):
        return self in [Piece.R_KING, Piece.B_KING]

    def name(self):
        names = {
            self.EMPTY: 'empty',
            self.R_PAWN: 'redpawn',
            self.B_PAWN: 'bluepawn',
            self.R_KING: 'redking',
            self.B_KING: 'blueking',
        }
        return names[self]
